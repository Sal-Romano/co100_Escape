...
// ATR Revive - Player damage multiplier (required for HandleDamage to work)
AT_Revive_PlayerDamage = 0.60;
publicVariable "AT_Revive_PlayerDamage";

// Force intel system on (in case params haven't propagated yet)
if(isNil "A3E_Param_UseIntel") then {
	A3E_Param_UseIntel = 1;
	publicVariable "A3E_Param_UseIntel";
};
if(isNil "A3E_Param_IntelChance") then {
	A3E_Param_IntelChance = 50;
	publicVariable "A3E_Param_IntelChance";
};
...

        {
            ...
            if (!_anyoneUp) then {
                ...
                [_grp, _activeMembers] spawn {
                    ...
                    // === FULL INLINE WIPE HANDLER ===

                    // 1. Server-side: reset ALL state immediately
                    {
                    ...
                    // 3. Black screen
                    {["", "BLACK", 1] remoteExec ["cutText", _x]} forEach _members;
                    sleep 2;

                    // 4. ESCAPE FAILED title
                    ...
                    // Non-leaders: wait for leader
                    {
                        if (_x != _leader) then {
                            [[], {
                                titleText [format ["Waiting for %1 to select spawn...", name (leader (group player))], "BLACK", 0.5];
